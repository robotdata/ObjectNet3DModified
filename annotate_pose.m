...
% Last Modified by GUIDE v2.5 30-Jul-2020 15:56:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
% Variable single ton initialisation
...
if exist(cachefile, 'file')
    ...
    num_objects = length(record.objects);
        ...
        prompt = 'You are using a different annotation directory, would you like to delete the previous cache and rerun?';
        usr_inp = inputdlg(prompt);
    end
else
    img_pos = 1;
    obj_pos = 1;
...
while img_pos <= num_anno_files && obj_pos <= num_objects && flag == 0
    ...
    % bbox and cad
    bbox = object.bbox;
    % Reads the bounding box from here
...
if flag == 0
    ...
    handles.width = width;
    handles.name = filename(1:end-4); %CONSIDERING last 4 characters as JPEG/handles.name will have n00000.%
...
disp(fprintf("Class index %d",handles.cls_ind))
disp(handles.cls_ind);
disp(handles.cls);
disp(find(strcmp(class_names,handles.cls) == 1));
disp(handles.cad_index);
% validity check
if isempty(handles.cad_index)
    handles.cad_index = 1;
end

% display one cad model in the current axis
faces = handles.cads{handles.cls_ind}(handles.cad_index).faces;
if isfield(handles.cads{handles.cls_ind},'face4') == 1
    face4 = handles.cads{handles.cls_ind}(handles.cad_index).face4;
end
...
if isfield(handles.cads{handles.cls_ind},'face4') == 1
    quadmesh(face4,vertices(:,1),vertices(:,2),vertices(:,3), 'EdgeColor','b');
    disp('Done with quadmesh');
end
hold off
...
disp(get(handles.togglebutton_display_overlay,'Max'));
...
while img_pos <= num_anno_files && obj_pos <= num_objects && flag == 0
    ...
    if isfield(handles.cads{cls_ind},'face4') == 1
        f4 = handles.cads{cls_ind}(cad_index).face4;
    end
    trimesh(faces, vertices(:,1), vertices(:,2), vertices(:,3), 'EdgeColor', 'b');
    hold on;
    if isfield(handles.cads{cls_ind},'face4') == 1
         quadmesh(f4, vertices(:,1),vertices(:,2),vertices(:,3),'edgecolor','r');
    end
...
if exist(matfile, 'file')    
    ...
    record.objects(handles.objpos).bbox = handles.bbox;
...
if get(hObject, 'Value') == get(hObject, 'Max')
    ...
    if isfield(handles.cads{handles.cls_ind},'face4') == 1
        f4 = handles.cads{handles.cls_ind}(handles.cad_index).face4;
    end
...
v = handles.cads{handles.cls_ind}(handles.cad_index).vertices;
f = handles.cads{handles.cls_ind}(handles.cad_index).faces;
if isfield(handles.cads{handles.cls_ind},'face4')
    f4 = handles.cads{handles.cls_ind}(handles.cad_index).face4;
end
x2d = project_3d(v, handles.azimuth, handles.elevation, handles.distance, ...
    handles.focal, handles.theta, [handles.px handles.py], handles.viewport);
...
bbox = handles.bbox;
% bbox(1) = bbox(1)-step_size;
% bbox(3) = bbox(3)-step_size;
handles.bbox = bbox;
guidata(hObject, handles);
...
bbox = handles.bbox;
% bbox(2) = bbox(2)- step_size;
% bbox(4) = bbox(4) - step_size;
handles.bbox = bbox;
guidata(hObject, handles);
...
%bbox = handles.bbox;
%bbox(1) = bbox(1)*exp(-get(hObject,'Value'));
%handles.bbox = bbox;
guidata(hObject, handles);
% display new overlay
if handles.overlay_on
...
% --- Executes on button press in IncrementX1.
function IncrementX1_Callback(hObject, eventdata, handles)
% hObject    handle to IncrementX1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in DecrementX1.
function DecrementX1_Callback(hObject, eventdata, handles)
% hObject    handle to DecrementX1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in DecrementY1.
function DecrementY1_Callback(hObject, eventdata, handles)
% hObject    handle to DecrementY1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.height);
step_size = handles.height/80;
bbox(2) = bbox(2)+step_size;
handles.bbox = bbox;
guidata(hObject, handles);
disp(handles.objpos);
...
% --- Executes on button press in IncrementY1.
function IncrementY1_Callback(hObject, eventdata, handles)
% hObject    handle to IncrementY1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.height);
...
% --- Executes on button press in IncrementX2.
function IncrementX2_Callback(hObject, eventdata, handles)
% hObject    handle to IncrementX2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.height);
...
% --- Executes on button press in DecrementX2.
function DecrementX2_Callback(hObject, eventdata, handles)
% hObject    handle to DecrementX2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.width);
...
% --- Executes on button press in DecrementY2.
function DecrementY2_Callback(hObject, eventdata, handles)
% hObject    handle to DecrementY2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.height);
...
% --- Executes on button press in IncrementY2.
function IncrementY2_Callback(hObject, eventdata, handles)
% hObject    handle to IncrementY2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
bbox = handles.bbox;
disp(handles.height);
...
% --- Executes on button press in imgNext.
function imgNext_Callback(hObject, eventdata, handles)
% hObject    handle to imgNext (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in prvImage.
function prvImage_Callback(hObject, eventdata, handles)
% hObject    handle to prvImage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
anno_dir = handles.anno_dir;
files = handles.files;
